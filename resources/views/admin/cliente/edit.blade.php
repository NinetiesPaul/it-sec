...
@section('editing_form')
    ...
    <form action="/admin/client/{{$user->id}}" method="post" role="form" class="form-horizontal " >
        @csrf
            ...
            <div class="col-md-3">
                ...
                <select name="area_id" id="area_id" class="form-control form-control-sm selectpicker" title="Select an area">
                    @foreach($areas as $area)
                        <option value="{{$area->id}}" {{($area->id === $user->isClient->area_id) ? 'selected' : '' }} >{{$area->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
