...
@section('form')
    <p><strong>Atribuir agente a {{$veiculo->model}} {{$veiculo->year}} {{$veiculo->license}}</strong></p>
    <form action="/admin/vehicle/{{$veiculo->id}}/assign" method="post" role="form" class="form-horizontal " >
        @csrf
...
@section('listing')
    <p><strong>Historico de Uso de {{$veiculo->model}} {{$veiculo->year}} {{$veiculo->license}}</strong></p>
    <table class="table">
